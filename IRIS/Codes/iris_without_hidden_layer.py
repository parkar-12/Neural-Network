...
def initialize_network(no_inputs,no_output):
	weight = list()
	inp = [{'weights':[(random.randint(1,100)/100) for i in range(no_inputs+1)]} for i in range(no_output)]
	weight.append(inp)
	return weight
...
def algorithm(data,no_inputs,no_output,no_of_generations,no_of_tests):
	network = initialize_network(no_inputs,no_output)
...
if __name__=="__main__":
	...
	no_of_tests = 5
	accuracy = algorithm(data,no_inputs,no_output,no_of_generations,no_of_tests)